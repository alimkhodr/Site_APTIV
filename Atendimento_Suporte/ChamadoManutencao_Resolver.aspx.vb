Public Class ChamadoManutencao_Resolver
    ...
    Protected Sub cmdsalvar_Click(sender As Object, e As EventArgs) Handles cmdsalvar.Click
        If idchamado <> "" Then
            conectabdnew()
            comandoSQL.CommandText = "SELECT * FROM TAB_CHAMADO_MANUTENCAO WHERE ID = @ID"
            ...
            If objDataReader.Read() Then
                If objDataReader("DATA_ANDAMENTO") Is DBNull.Value Then
                    ...
                    comandoSQL.CommandText = "UPDATE TAB_CHAMADO_MANUTENCAO SET DATA_ANDAMENTO = GETDATE() WHERE ID = @ID"
                    comandoSQL.Parameters.AddWithValue("@ID", idchamado)
                ...
                Else
                    ...
                    Else
                        objDataReader.Close()
                        comandoSQL.Parameters.Clear()
                        comandoSQL.CommandText = "UPDATE TAB_CHAMADO_MANUTENCAO Set RESOLUCAO = @RESOLUCAO, REGISTRO_RESOLUCAO = @REGISTRO_RESOLUCAO, DATA_RESOLUCAO = GETDATE() WHERE ID = @ID"
    ...
    Private Sub PreencherChamado()
        ...
        conectabdnew()
        comandoSQL.CommandText = "SELECT TAB_CHAMADO_MANUTENCAO.*, FUNCIONARIO.FUN_NOME FROM TAB_CHAMADO_MANUTENCAO INNER JOIN FUNCIONARIO ON FUNCIONARIO.FUN_REGISTRO = TAB_CHAMADO_MANUTENCAO.REGISTRO_CHAMADO WHERE TAB_CHAMADO_MANUTENCAO.DATA_RESOLUCAO IS NULL"
        objDataReader = comandoSQL.ExecuteReader
    ...
    Protected Sub btnaddmanu_Click(sender As Object, e As EventArgs) Handles btnaddmanu.Click
        If txtmanu.Text = "" Then
            ORDEMP.Text = "Tipo de chamado vazio"
            mp1.Show()
        End If
        Try
            conectabdnew()
            comandoSQL.CommandText = "INSERT INTO TAB_CHAMADO_MANUTENCAO_TIPO VALUES (@TIPO)"
            comandoSQL.Parameters.AddWithValue("@TIPO", txtmanu.Text)
            comandoSQL.ExecuteReader()
            fechabdnew()
            ORDEMP.Text = "Tipo de chamado cadastrado com sucesso!"
            mp1.Show()
        Catch ex As Exception
            ORDEMP.Text = "Tipo de manutenção " & txtmanu.Text & " já cadastrado"
            mp1.Show()
            Exit Sub
        End Try
    End Sub
